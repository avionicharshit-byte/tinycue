// EdgeNLU: read typed commands over the serial port, fully offline.
//
...
//
// The model bundled here is the toy coffee machine that `edgenlu init` writes, so it
...
static void handle(const char *text) {
  ...
  int code = enlu_parse(&model, text, &answer, scratch, sizeof scratch);
  unsigned long taken = micros() - started;
  if (code != ENLU_OK) {
    Serial.print("refused: ");
    Serial.println(enlu_error(code));
    return;
  }
  report(taken);
}
...
void setup() {
  ...
  int code = enlu_init(&model, enlu_model_data, enlu_model_data_len);
  if (code != ENLU_OK) {
  ...
  size_t needed = enlu_scratch_size(&model);
