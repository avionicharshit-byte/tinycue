/* edge-nlu device runtime. See edgenlu.h, docs/model-format.md and features.py. */
#include "edgenlu.h"
                                   ...
                                   uint32_t want, uint32_t *size)
{
    uint32_t i;
    for (i = 0; i < count; i++) {
        const uint8_t *entry = blob + ENLU_HEADER_SIZE + (size_t)i * ENLU_DIR_ENTRY;
...
int enlu_init(enlu_model *model, const uint8_t *blob, size_t len)
{
    ...
    if (model == NULL || blob == NULL) {
        return ENLU_BAD_ARGUMENT;
    }
    if (((uintptr_t)blob & 7u) != 0u) {
        return ENLU_BAD_BLOB;
    }
    if (len < ENLU_HEADER_SIZE + ENLU_DIR_ENTRY) {
        return ENLU_BAD_BLOB;
    }
    if (blob[0] != ENLU_MAGIC0 || blob[1] != ENLU_MAGIC1 || blob[2] != ENLU_MAGIC2 ||
        blob[3] != ENLU_MAGIC3) {
        return ENLU_BAD_BLOB;
    }
    if (read_u32(blob + 4) != ENLU_BLOB_VERSION) {
        return ENLU_BAD_BLOB;
    }
    ...
    if (model->label_count == 0u || model->label_count > ENLU_MAX_LABELS) {
        return ENLU_BAD_BLOB;
    }
...

    {
        ...
        for (c = 0; c < model->command_count; c++) {
            if (model->commands[(size_t)c * CMD_WORDS + 1] > (uint32_t)ENLU_MAX_SLOTS) {
                return ENLU_BAD_BLOB;
            }
        }
    }
...
typedef struct {
    char text[ENLU_MAX_TEXT];
    const char *token[ENLU_MAX_TOKENS];
    int token_len[ENLU_MAX_TOKENS];
    int count;
    char raw[ENLU_RAW_MAX];
    int raw_len;
    uint32_t bucket[ENLU_MAX_FEATURES];
    uint32_t repeats[ENLU_MAX_FEATURES];
    int bucket_count;
...
{
    size_t labels = model == NULL ? ENLU_MAX_LABELS : model->label_count;
    size_t lattice = (size_t)ENLU_MAX_TOKENS * labels;
    return round_up8(sizeof(enlu_work)) + round_up8(lattice * sizeof(double)) * 2u +
           round_up8(2u * labels * sizeof(double)) + round_up8(lattice);
}

/* -------------------------------------------------------------- tokenising */

static int tokenize(const char *text, enlu_work *work)
{
    size_t used = 0;
    const char *s = text;
    ...
    while (*s != '\0') {
        ...
        if (work->count >= ENLU_MAX_TOKENS) {
            return ENLU_TOO_LONG;
        }
        start = work->text + used;
        while (*s != '\0' && !is_space(*s)) {
            if (used + 2u > (size_t)ENLU_MAX_TEXT) {
                return ENLU_TOO_LONG;
            }
...
static int is_filler_word(const enlu_model *model, const char *word, int len)
{
    uint32_t i;
...
static int lookup_number_word(const enlu_model *model, const char *word, int len,
                              int32_t *value)
{
    uint32_t low = 0;
...
static int read_number(const enlu_model *model, const enlu_work *work, int start, int end,
                       int32_t *out)
{
    long total = 0;
    int seen = 0;
...
static int is_number_token(const enlu_model *model, const enlu_work *work, int index)
{
    int32_t value;
...
static void intent_scores(const enlu_model *model, enlu_work *work, double *scores)
{
    ...
    for (i = 0; i + 3 <= work->raw_len; i++) {
        ...
        if (work->bucket_count >= ENLU_MAX_FEATURES) {
            break;
        }
        work->bucket[work->bucket_count++] = h & mask;
    }
...
static void add_attribute(const enlu_model *model, uint64_t hash, double *row)
{
    uint32_t low = 0;
...
static int in_gazetteer(const enlu_model *model, uint32_t type, const char *word, int len)
{
...
static void token_state(const enlu_model *model, const enlu_work *work, int index,
                        double *row)
{
    uint32_t j;
    int o;
...
static double tag_sentence(const enlu_model *model, enlu_work *work)
{
...
static int join_tokens(const enlu_work *work, int start, int end, char *out, int cap)
{
    int n = 0;
    int i;
    for (i = start; i < end; i++) {
...
static const char *find_canonical(const enlu_model *model, uint32_t type, const char *text)
{
    const uint32_t *record = model->slot_types + (size_t)type * TYPE_WORDS;
    uint32_t v;
...
static enlu_match span_value(const enlu_model *model, enlu_work *work, uint32_t type,
                             int start, int end)
{
    const uint32_t *record = model->slot_types + (size_t)type * TYPE_WORDS;
    enlu_match out;
...
static enlu_match trim_span(const enlu_model *model, enlu_work *work, uint32_t type,
                            int *start, int *end)
{
    enlu_match out;
    int stop;
...
static int command_slot_for_type(const enlu_model *model, const uint32_t *command,
                                 uint32_t type)
{
    uint32_t i;
...
static void decode_tags(const enlu_model *model, enlu_work *work, uint32_t command_index,
                        enlu_result *out)
{
    const uint32_t *command = model->commands + (size_t)command_index * CMD_WORDS;
    int filled[ENLU_MAX_SLOTS];
    uint32_t i;
    int t;
    int current = -1;
    int span_start = 0;

    for (i = 0; i < ENLU_MAX_SLOTS; i++) {
        filled[i] = 0;
    }
    ...
    for (t = 0; t <= work->count; t++) {
        ...
        if (current >= 0 && (t == work->count || type < 0 || begins || type != current)) {
            ...
            if (slot_index >= 0 && !filled[slot_index] && out->slot_count < ENLU_MAX_SLOTS) {
                const uint32_t *slot =
                    model->command_slots + (size_t)(command[2] + (uint32_t)slot_index) *
                                               CMDSLOT_WORDS;
                int from = span_start;
                int to = t;
                enlu_match match = trim_span(model, work, (uint32_t)current, &from, &to);
                const uint32_t *record =
                    model->slot_types + (size_t)current * TYPE_WORDS;
                int keep = 1;
                enlu_slot *filled_slot = &out->slots[out->slot_count];
                ...
                if (keep) {
                    ...
                    if (match.is_number) {
                        ...
                        join_tokens(work, span_start, t, store, ENLU_MAX_SURFACE);
    ...
    for (i = 0; i < command[1]; i++) {
        const uint32_t *slot = model->command_slots + (size_t)(command[2] + i) * CMDSLOT_WORDS;
        if (slot[2] != 0u && !filled[i] && out->missing_count < ENLU_MAX_SLOTS) {
               ...
               size_t scratch_len)
{
    enlu_work *work;
    uint8_t *cursor;
    double scores[ENLU_MAX_LABELS > 64 ? ENLU_MAX_LABELS : 64];
    uint32_t labels;
    size_t lattice;
    uint32_t best = 0;
    uint32_t k;
    ...
    if (scratch_len < enlu_scratch_size(model)) {
        return ENLU_SMALL_SCRATCH;
    }
    ...
    lattice = (size_t)ENLU_MAX_TOKENS * labels;
    work = (enlu_work *)scratch;
    cursor = (uint8_t *)scratch + round_up8(sizeof(enlu_work));
