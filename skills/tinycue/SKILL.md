...
description: >
  ...
  sentences, and tinycue trains a tiny intent model plus a slot tagger and exports
  ...
  slow or too expensive, or whenever tinycue or an on-device intent parser
  is mentioned. Covers the file formats, the improvement loop driven by
  `tinycue doctor`, and dropping the runtime into firmware.
---

# Build an offline command parser with tinycue
...
```bash
tinycue doctor camera.yaml --extra camera.extra.yaml --dev camera.dev.yaml --json
```
...
```bash
tinycue train camera.yaml --extra camera.extra.yaml --dev camera.dev.yaml -o out/model
...
a C array, plus `tinycue.c` and `tinycue.h` copied out of the installed package. Add the
`.c` files to the firmware build. No dependencies beyond libm. For Arduino, the library is
`arduino/TinyCue` in the repository.

```c
#include "tinycue.h"
#include "model_data.h"
static tcue_model model;                       /* pointers into flash, no copy */
tcue_init(&model, tcue_model_data, tcue_model_data_len);
static uint8_t scratch[6000];                  /* >= tcue_scratch_size(&model) */
tcue_result r;
if (tcue_parse(&model, line, &r, scratch, sizeof scratch) == TCUE_OK && !r.unsure) {
...
|---|---|
| `tinycue check FILE [--extra F]` | validate and summarise, before training |
| `tinycue train FILE [--extra F] [--dev F] [-n 1500] [--cutoff-target 0.97] -o DIR` | train, calibrate, save |
| `tinycue doctor FILE [--extra F] --dev F [--json]` | train and say where it is weak |
| `tinycue eval DIR --data FILE [--summary]` | measure on a held-out set |
| `tinycue init NAME` | write a starter commands file, extra file and dev file |
| `tinycue export DIR -o DIR [--with-runtime]` | write `model.bin`, the C arrays and, with the flag, the runtime |
| `tinycue parse DIR "sentence"` | try one sentence on the desktop |
| `tinycue generate FILE -n 200` | dump the training sentences as JSON lines |
...
`tinycue eval` reads a set written answer first as well as the older marked-up layout.
...
`tinycue parse DIR --json "sentence"` prints the answer with the evidence behind the
