"""The tinycue command line: init, check, generate, train, doctor, eval, export, parse."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--version", action="version", version=f"tinycue {__version__}")
    ...
    ex.add_argument(
        "--with-runtime",
        action="store_true",
        help="copy tinycue.h and tinycue.c in beside the model, so the folder builds on its own",
    )
...
def _init(args) -> int:
    ...
    print()
    print("next:")
    print(f"  tinycue check {spec} --extra {extra}")
    print(f"  tinycue train {spec} --extra {extra} --dev {dev} -o out/model")
    print('  tinycue parse out/model "make me two lattes"')
    print("  tinycue export out/model -o out/device --with-runtime")
    return 0
...
def _doctor(args) -> int:
    ...
    if args.out:
        ...
        with tempfile.TemporaryDirectory(prefix="tinycue-doctor-") as temp:
